						...
						for gb_temp in ${gb_temp_bpr[@]}; do
							for hyper_beta in 0.02; do
								for w2v_ratio in 0.1; do
									cur_dt="`date +%m%d`"
									expname=tt_tsne_bt${tsne_bt}hyp${hyper_beta}${w2v_ratio}_gb${gb_temp}_tt_d${dis_1}_${dis_2}_${dis_3}_mt${mt_ratio}_dm${dis_mu}_hyper_lr${lr}
									cmd="python3 ./main.py --sumout ./neva_22_tsne/outpt${cur_dt}_ts_w2v_tse_plt_10/ts_w2v/${DATASET}/${expname}/ \
										...
										--dataset $DATASET  --aspect_tsne_bt ${tsne_bt} --train_type ts_only_neva_tsne --student_type w2v_rec_tsne --lr ${lr} \
