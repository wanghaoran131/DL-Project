...
gl_temp_list=(1e-02 1e-03 1e-04)

for lr in 0.00000035; do
	for dis_1 in 10; do
		for dis_2 in 10.0; do
			for dis_3 in 50; do
				for mt_ratio in 10; do
				# for mt_ratio in 100; do
					for gb_temp in ${gl_temp_list[@]}; do
						for hyper_beta in 0.02; do
							for w2v_ratio in 0.1; do
								...
								cmd="python3 ./main.py --sumout ./neva_13/outpt${cur_dt}_dis_mt_gb_pgt/hyp_dis_rec_mt_10/${DATASET}/${expname}/ \
									...
									--dataset $DATASET --train_type rec_mt_neva --student_type hyper_rec_dis_10_gbl --lr ${lr} \
