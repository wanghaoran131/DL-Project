					...
					for gb_temp in ${gb_temp_bpr[@]}; do
						for hyper_beta in 0.02; do
							for w2v_ratio in 0.1; do
								cur_dt="`date +%m%d`"
								expname=tsb_${tsne_bt}p${hyper_beta}${w2v_ratio}_gb${gb_temp}d${dis_1}${dis_2}${dis_3}mt${mt_ratio}dm${dis_mu}lr${lr}
								cmd="python3 ./main.py --sumout ./neva_23_tsne/outpt${cur_dt}_dis_mt_gb_tse_plt_5/hyp_dis_rec_mt_10/${DATASET}/${expname}/ \
									...
									--dataset $DATASET --aspect_tsne_bt ${tsne_bt} --train_type rec_mt_neva_tsne --student_type hyper_rec_dis_10_gbl_tsne --lr ${lr} \
...
# 'hyper_rec_dis_10_gbl_tsne', 'w2v_rec_tsne', 'w2v_tsne',