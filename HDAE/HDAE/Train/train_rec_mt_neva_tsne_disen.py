...
class Trainer:
    ...
    def train_per_epoch(self, loader):
        ...
        for x_bow, x_id, ori in loader:
            ...
            x_bow, x_id, ori = x_bow[0,:,:], x_id[0,:,:], ori[0]
    ...
    def write_result(self, s_logits, grum_a_probs, ori, file_name = 'trn'):

        ori_tmp = [ori_sent[0] for ori_sent in ori]
        ...
        for lg_i in range(len(lg)):
            ...
            file_grum_probs.write('Sent = ' + ori_tmp[lg_i] + '\n')
    ...
    def train_step(self, x_bow, x_id, ori, bt_times):
        # apply teacher
        ...
        eu_r, _, s_logits, hyper_sent, grum_a_probs = self.student(x_id)
        
        self.write_result(s_logits, grum_a_probs, ori, 'trn')
    ...
    def test(self, epoch):
        ...
        file_grum_probs = open(self.hparams.sumout_grum_probs + '/disa_grum_{:d}.txt'.format(epoch), "w")
        ...
        for batch in self.test_loader:
            ...
            # print('bow = ', bow.shape)
            # input() 
            bow, idx, labels, ori = bow[0,:,:,:], idx[0,:,:], labels[0,:,:], ori[0]
            ...
            with torch.no_grad():
                _, _, logits, hyper_sent, grum_a_probs = self.student(idx)

            # dis aspect string
            # grum_a_probs_tmp = grum_a_probs.detach().cpu().tolist()


            self.write_result(logits, grum_a_probs, ori, 'test')

            # print('grum_a_probs_lb = ', grum_a_probs_lb)
            # print('grum_a_probs_lb = ', len(grum_a_probs_lb))
            # input()
