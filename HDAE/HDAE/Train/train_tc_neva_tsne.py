...
class Trainer:
    ...
    def train_step(self, x_bow, x_id, x_ori, bt_times):
        ...
        eu_r, _, s_logits, eu_sent = self.student(x_id)

        eu_sent_detech =  eu_sent.detach().cpu().tolist()

        # if bt_times < self.hparams.aspect_tsne_bt:
        #     for lg_i in range(len(eu_sent_detech)):
        #         # print('eu_sent_detech[lg_i] = ', eu_sent_detech[lg_i], len(eu_sent_detech[lg_i]))
        #         # input()
        #         self.total_sentence_vec.append(eu_sent_detech[lg_i])

        if bt_times < self.hparams.aspect_tsne_bt:
            for lg_i in range(len(eu_sent_detech)):
                self.total_sentence_vec.append(eu_sent_detech[lg_i])
        else:
            self.total_sentence_vec = self.total_sentence_vec[len(eu_sent_detech):]
            for lg_i in range(len(eu_sent_detech)):
                self.total_sentence_vec.append(eu_sent_detech[lg_i])
    ...
    def test(self, epoch):
        ...
        for batch in self.test_loader:
            ...
            for lg_i in range(len(lg)):
                ...
                # print('hyper_sent_detech[lg_i] = ', hyper_sent_detech[lg_i], len(hyper_sent_detech[lg_i]))
                # input()
