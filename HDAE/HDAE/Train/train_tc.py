...
class Trainer:
    def __init__(self, hparams) -> None:
        ...
        test_ds = TestDataset(hparams, hparams.aspect_init_file, hparams.test_file, hparams.st_pre, hparams.maxlen)
    ...
    def test(self, epoch):
        ...
        for batch in self.test_loader:
            ...
            with torch.no_grad():
                t_logits = self.teacher(bow, self.z)
                # logits = self.student(idx)
            
            lg = t_logits.max(-1)[1].detach().cpu().tolist()
        ...
        score = f1_score(y_true=ref, y_pred=pred)
        print('f1 = ',score)
