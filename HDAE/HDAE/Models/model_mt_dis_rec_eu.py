...
class Student_Hyp_DIS_REC(nn.Module):
    def __init__(self, hparams, neg_samples = 10, bias=True, M=None, b=None) -> None:
        ...
        self.manifold_eu = getattr(manifolds, "Euclidean")()
    ...
    def student_init(self):
        ...
        # xavier_uniform(self.a_dis_emb.data)
    ...
    def intra_aspect_model(self):
        ...
        hyp_a_dis_multi_o2a = self.manifold_eu.proj_tan0_exp(a_dis_multi_o2a.view(-1, self.emb_size), 1)

        hyp_a_dis_multi_rpa = self.manifold_eu.proj_tan0_exp(a_dis_multi_rpa.view(-1, self.emb_size), 1)

        as_seed_dis_inter_dis = self.manifold_eu.sqdist(hyp_a_dis_multi_o2a, hyp_a_dis_multi_rpa, 1)

        # print('1 as_seed_dis_inter_dis = ', as_seed_dis_inter_dis.shape)
        ...
        # print('2 as_seed_dis_inter_dis = ', as_seed_dis_inter_dis.shape)
        ...
        # print('1 inter_as_sd_inter_dis = ', inter_as_sd_inter_dis.shape)
        # print('inter_as_sd_inter_dis = ', inter_as_sd_inter_dis[1, : 10, : 10])
        # input()
        ...
        # print('2 inter_as_sd_inter_dis = ', inter_as_sd_inter_dis.shape)

        # print('inter_as_sd_inter_dis_pre = ', inter_as_sd_inter_dis_pre[1, : 5, :, : 5, :])
        # input()

        inter_as_sd_inter_dis, _ = torch.min(inter_as_sd_inter_dis_pre, dim=4)

        # print('inter_as_sd_inter_dis = ', inter_as_sd_inter_dis[1, : 5, :, : 5])
        # input()
        # print('3 inter_as_sd_inter_dis = ', inter_as_sd_inter_dis.shape)

        inter_as_sd_inter_dis, _ = torch.min(inter_as_sd_inter_dis, dim=2)
        # print('inter_as_sd_inter_dis = ', inter_as_sd_inter_dis[1, : 5, : 5])
        # input()

        # print('inter_as_sd_inter_dis = ', inter_as_sd_inter_dis, inter_as_sd_inter_dis.mean().detach().item())
        # input()

        dist_hinge = torch.clamp(inter_as_sd_inter_dis - self.dis_1, min=0.0)
        # print('dist_hinge = ', dist_hinge)
        # print('dist_hinge = ', dist_hinge.shape)
        # input()
        ...
        # print('dist_loss_1 = ', dist_loss_1)
        # print('dist_loss_1 = ', dist_loss_1.shape)
        # input()
        ...
        # print('intra_as_sd_intent_dis = ', intra_as_sd_intent_dis[0,:,:])
        # print('intra_as_sd_intent_dis = ', intra_as_sd_intent_dis.shape)
        # # input()
...
# 
        ...
        # print('dist_loss_2 = ', dist_loss_2)
        # print('dist_loss_2 = ', dist_loss_2.shape)
        # input()
        ...
        # repeat_a_emb_w = self.a_emb.unsqueeze(2).repeat(1, 1, self.dis_mun, 1)

        hyp_a_dis_emb = self.manifold_eu.proj_tan0_exp(self.a_dis_emb.view(-1, self.emb_size), 1)

        hyp_repeat_a_emb_w = self.manifold_eu.proj_tan0_exp(repeat_a_emb_w.view(-1, self.emb_size), 1)

        g_as_intent_dis = self.manifold_eu.sqdist(hyp_a_dis_emb.view(-1, self.emb_size), hyp_repeat_a_emb_w.view(-1, self.emb_size), 1)

        # print('1 g_as_intent_dis = ',g_as_intent_dis.shape)

        g_as_intent_dis = torch.clamp(g_as_intent_dis - self.dis_3 , min=0.0)

        # print('2 g_as_intent_dis = ',g_as_intent_dis.shape)
    ...
    def aspect_intent_sel(self, enc_aspects):
        ...
        hyp_as_sd_den = self.manifold_eu.proj_tan0_exp(as_sd_den.view(-1, dim), 1)

        bz_hyp_as_sd_den = (hyp_as_sd_den.unsqueeze(0)).repeat(bsz,1,1)

        probs_pre = self.manifold_eu.sqdist(enc_aspects.view(-1, dim), bz_hyp_as_sd_den.view(-1, dim), 1)

        probs_pre = probs_pre.view(-1,1)

        # print('probs_pre = ', probs_pre[:10,:])
        # input()
        ...
        # print('11grum_a_probs seed 0 = ', pre_a_probs[0,0,0,:])
        # print('1 grum_a_probs seed 1 = ', pre_a_probs[0,0,1,:])
        # input()

        pre_a_probs = pre_a_probs / self.gb_temp

        # print('2 grum_a_probs seed 0 = ', pre_a_probs[0,0,0,:])
        # print('2 grum_a_probs seed 1 = ', pre_a_probs[0,0,1,:])
        # input()
        
        grum_a_probs = F.softmax(pre_a_probs, dim=3)

        # print('grum_a_probs seed 0 = ', grum_a_probs[0,0,0,:])
        # print('grum_a_probs seed 1 = ', grum_a_probs[0,0,1,:])
        # input()
    ...
    def forward(self, inputs, batch_num=None):
        ...
        enc = self.manifold_eu.proj_tan0_exp(enc.view(-1, dim), 1)
        ...
        # print('1 bz_a_emb_w = ', bz_a_emb_w.shape)

        bz_a_emb_w = bz_a_emb_w.sum(dim=2)
        # bz, numaspect, -1

        # print('2 bz_a_emb_w = ', bz_a_emb_w.shape)
        # input()

        r = a_probs.view(bsz, self.hparams.st_num_aspect, 1).mul(bz_a_emb_w).sum(dim=1)

        # print('a_probs.view(bsz, self.hparams.st_num_aspect, 1).mul(bz_a_emb_w) = ', a_probs.view(bsz, self.hparams.st_num_aspect, 1).mul(bz_a_emb_w).shape)
        # print('r shape = ', r.shape)
        # input()

        return r, a_probs, a_probs
