...
class Student_Hyp_DIS_REC(nn.Module):
    ...
    def student_init(self):
        ...
        dis_a_emb = dis_a_emb + torch.randn(dis_a_emb.size()) * 0.0005
    ...
    def intra_aspect_model(self):
        ...
        # print('self.a_dis_emb = ', self.a_dis_emb[0,:4,:,:])
        # input()
        ...
        # print('a_dis_multi_o2a')
        # print('a_dis_multi_o2a = ', a_dis_multi_o2a.shape)
        ...
        # print('a_dis_multi_rpa = ', a_dis_multi_rpa.shape)
...
# 
        ...
        # g_as_intent_dis = torch.clamp(g_as_intent_dis - self.margin, min=0.0)

        # # print('2 g_as_intent_dis = ',g_as_intent_dis.shape)

        # dist_loss_3 = torch.mean(g_as_intent_dis)

        # return dist_loss_1 + dist_loss_2 + dist_loss_3

        return dist_loss_1 + dist_loss_2
    ...
    def aspect_intent_sel(self, enc_aspects):
        ...
        pre_a_probs_zero = torch.zeros(pre_a_probs.size()).to(pre_a_probs.device)

        tmp = torch.argmax(pre_a_probs, dim=3).unsqueeze(-1)
        ...
        pre_a_probs_zero[mask == True] = 1

        pre_a_probs_zero = (pre_a_probs_zero / (pre_a_probs_zero.sum(3).unsqueeze(3)))

        bz_a_dis_emb = (self.a_dis_emb.unsqueeze(0)).repeat(bsz, 1, 1, 1, 1)

        a_dis_emb_tmp = bz_a_dis_emb.view(bsz, self.num_aspects, self.num_seeds, self.dis_mun, -1) * pre_a_probs_zero.view(bsz, self.num_aspects, self.num_seeds, self.dis_mun, 1)
