...
def distan_margin_cosine_loss(positive, negative, margin=0.5, eps=1e-8, sum_loss=False):
    if negative.dim() == 2:
        maxi = torch.log(torch.sigmoid(negative - positive))
        base_loss = torch.negative(torch.mean(maxi, 1))
    else:
        maxi = torch.log(torch.sigmoid(negative - positive))
        base_loss = torch.negative(torch.mean(maxi))

    if not sum_loss:
        loss = torch.mean(base_loss)
    else:
        loss = torch.sum(base_loss)
    return loss


class TripletMarginCosineLoss_hyperdis(Module):
    ...
    def __init__(self, margin=1.0, eps=1e-8, sum_loss=False):
        super(TripletMarginCosineLoss_hyperdis, self).__init__()
        self.margin = margin
        self.eps = eps
        self.sum_loss = sum_loss

    def forward(self, positive, negative):
        return distan_margin_cosine_loss(positive, negative, self.margin, self.eps, self.sum_loss)
